...
contract JIO is ERC20Permit, Pausable, SimpleGovernance {
    ...
    /**
     * @notice Initializes JIO token with specified governance address and recipients. For vesting
    ...
    constructor(
    ...
    ) public ERC20("Jioswap DAO", "JIO") ERC20Permit("Jioswap DAO") {
        require(governance_ != address(0), "JIO: governance cannot be empty");
        require(
            vestingContractTarget_ != address(0),
            "JIO: vesting contract target cannot be empty"
        );
        require(
            pausePeriod_ > 0 && pausePeriod_ <= 10 minutes,
            "JIO: pausePeriod must be in between 0 and 10 min"
        );
        ...
        anyoneCanUnpauseAfter = block.timestamp + 10 minutes;
     ...
     * release schedule can be found on https://docs.jioswap.finance
        ...
        returns (address)
    {
        require(
            ...
            "JIO: duration for vesting cannot be 0"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused() || allowedTransferee[from], "JIO: paused");
        require(to != address(this), "JIO: invalid recipient");
    }
    ...
    function rescueTokens(
        ...
        require(to != address(0), "JIO: invalid recipient");
        ...
        if (token == IERC20(address(0))) {
            ...
            require(balance > 0, "JIO: trying to send 0 ETH");
            // slither-disable-next-line arbitrary-send
            (bool success, ) = to.call{value: balance}("");
            require(success, "JIO: ETH transfer failed");
        } else {
            // any other erc20
            ...
            balance = balance == 0
                ? totalBalance
                : Math.min(totalBalance, balance);
            require(balance > 0, "JIO: trying to send 0 balance");
