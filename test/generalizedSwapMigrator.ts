...
describe("GeneralizedSwapMigrator", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      oldUSDPool = await ethers.getContract("JioUSDPool")
      oldUSDPoolLPToken = await ethers.getContract("JioUSDPoolLPToken")
      newUSDPool = await ethers.getContract("JioUSDPoolV2")
      newUSDPoolLPToken = await ethers.getContract("JioUSDPoolV2LPToken")
      ...
      for (const token of [DAI, USDC, BUSD]) {
    ...
    it("Successfully updates migrationMap", async () => {
      await genSwapMigrator.addMigrationData(
        oldUSDPool.address,
        {
          ...
          tokens: [DAI.address, USDC.address, BUSD.address],
        },
        true,
      )
    })
