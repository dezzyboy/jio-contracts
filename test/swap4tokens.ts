...
describe("Swap with 4 tokens", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      JIO = await ethers.getContract("JIO")

      TOKENS.push(DAI, USDC, BUSD, JIO)
      ...
      await asyncForEach(
        ...
        async (address) => {
          ...
          await BUSD.mint(address, String(1e8))
          await JIO.mint(address, String(1e20))
        },
      )
      ...
      await swap.initialize(
        [DAI.address, USDC.address, BUSD.address, JIO.address],
        [18, 6, 6, 18],
        LP_TOKEN_NAME,
        LP_TOKEN_SYMBOL,
        INITIAL_A_VALUE,
      ...
      await asyncForEach([owner, user1, user2, attacker], async (signer) => {
        ...
        await JIO.connect(signer).approve(swap.address, MAX_UINT256)
      })
  ...
  describe("swap", () => {
    it("Swap works between tokens with different decimals", async () => {
      ...
      await BUSD.connect(user1).approve(swap.address, String(1e6))
      await swap
        .connect(user1)
        .swap(
          2,
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 16e6 of USDC to JIO, causing massive imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(16e6), 0, MAX_UINT256)
            const SUSDOutput = (await getUserTokenBalance(attacker, JIO)).sub(
              initialAttackerBalances[3],
            )

            // First trade results in 15.87e18 of JIO
            expect(SUSDOutput).to.be.eq("15873636661935380627")

            // Pool is imbalanced! Now trades from JIO -> USDC may be profitable in small sizes
            // USDC balance in the pool : 66e6
            // JIO balance in the pool : 34.13e18
            ...
            // Trade JIO to USDC, taking advantage of the imbalance and change of A
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Trade JIO to USDC, taking advantage of the imbalance and sudden change of A
            const balanceBefore = await getUserTokenBalance(attacker, USDC)
            await swap.connect(attacker).swap(3, 1, SUSDOutput, 0, MAX_UINT256)
            const USDCOutput = (await getUserTokenBalance(attacker, USDC)).sub(
              balanceBefore,
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swapping 25e6 of USDC to JIO, resolving imbalance in the pool
            await swap
              .connect(attacker)
            ...
            // First trade results in 25.14e18 of JIO
            // Because the pool was imbalanced in the beginning, this trade results in more than 25e18 JIO
            expect(SUSDOutput).to.be.eq("25140480043410581418")

            // Pool is now almost balanced!
            // USDC balance in the pool : 75.00e6
            // JIO balance in the pool : 74.86e18
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Swap 25e6 of USDC to JIO, resolving the imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(25e6), 0, MAX_UINT256)
            const SUSDOutput = (await getUserTokenBalance(attacker, JIO)).sub(
    ...
    describe("Check for attacks while A is ramping downwards", () => {
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 25e6 of USDC to JIO, resolving imbalance in the pool
            await swap
              .connect(attacker)
              .swap(1, 3, String(25e6), 0, MAX_UINT256)
            const SUSDOutput = (await getUserTokenBalance(attacker, JIO)).sub(
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // First trade results in 25.14e18 of JIO
            // Because the pool was imbalanced in the beginning, this trade results in more than 1e18 JIO
            expect(SUSDOutput).to.be.eq("25140480043410581418")
