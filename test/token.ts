...
import { JIO, Vesting } from "../build/typechain/"
...
describe("Token", () => {
  ...
  const setupTest = deployments.createFixture(
    async ({ deployments, ethers }) => {
      ...
      const determinedDeployment = await deterministic("JIO", {
        from: deployerAddress,
      ...
      // Find the jio token deployment
      jioToken = await ethers.getContract("JIO")

      // Approve the token usage for deploying vesting contracts
      await jioToken.connect(governance).approve(jioToken.address, MAX_UINT256)

      // Call `deployNewVestingContract` for each vesting recipient
      for (const vestingReciepient of vestingRecipients) {
        // Preview the address
        const cloneAddress = await jioToken
          .connect(governance)
      ...
      await jioToken.connect(governance).addToAllowedList(approvedTransferees)
    },
  )
  ...
  describe("minting", () => {
    it("Successfully mints to appropriate addresses and vestings", async () => {
      ...
      expect(await jioToken.balanceOf(await signers[10].getAddress())).to.eq(0)
      expect(await jioToken.balanceOf(await signers[11].getAddress())).to.eq(0)
      expect(await jioToken.balanceOf(await signers[12].getAddress())).to.eq(0)
      expect(await jioToken.balanceOf(vestingContracts[0].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await jioToken.balanceOf(vestingContracts[1].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await jioToken.balanceOf(vestingContracts[2].address)).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )

      // Non-vesting addresses
      expect(await jioToken.balanceOf(await signers[13].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
      expect(await jioToken.balanceOf(await signers[14].getAddress())).to.eq(
        BIG_NUMBER_1E18.mul(2e8),
      )
    })
  })
  ...
  describe("totalSupply", () => {
    it("Successfully mints max supply (1e9 with 1e18 decimals) on deployment", async () => {
      expect(await jioToken.totalSupply()).to.eq(BIG_NUMBER_1E18.mul(1e9))
    })
  })

  describe("govCanUnpauseAfter", () => {
    it("Successfully sets govCanUnpauseAfter to be in the future on deployment", async () => {
      expect(await jioToken.govCanUnpauseAfter()).to.eq(
  ...
  describe("anyoneCanUnpauseAfter", () => {
    it("Successfully sets anyoneCanUnpauseAfter to be 1 year after the deployment", async () => {
      expect(await jioToken.anyoneCanUnpauseAfter()).to.eq(
  ...
  describe("governance", () => {
    it("Successfully sets the governance address on deployment", async () => {
      expect(await jioToken.governance()).to.eq(governanceAddress)
    })
  })
  ...
  describe("transfer", () => {
    it("Successfully transfers from an allowed address when paused", async () => {
      ...
      expect(await jioToken.balanceOf(deployerAddress)).to.eq(
  ...
  describe("enableTransfer", () => {
    it("Reverts when governance attempts to unpause before govCanUnpauseAfter", async () => {
      expect(await jioToken.govCanUnpauseAfter()).to.gt(
        await getCurrentBlockTimestamp(),
      )
      await expect(
        jioToken.connect(governance).enableTransfer(),
      ).to.be.revertedWith("JIO: cannot enable transfer yet")
    })

    it("Reverts when non-governance attempts to unpause after govCanUnpauseAfter", async () => {
      await setTimestamp(
        (await jioToken.govCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await jioToken.govCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await expect(
        jioToken.connect(deployer).enableTransfer(),
      ).to.be.revertedWith("JIO: cannot enable transfer yet")
    })
    ...
    it("Succeeds when governance attempts to unpause after govCanUnpauseAfter", async () => {
      ...
      await jioToken.connect(governance).enableTransfer()
      expect(await jioToken.paused()).to.be.false
    })

    it("Succeeds when non-governance attempts to unpause after anyoneCanUnpauseAfter", async () => {
      await setTimestamp(
        (await jioToken.anyoneCanUnpauseAfter()).add(1).toNumber(),
      )
      expect(await jioToken.anyoneCanUnpauseAfter()).to.lte(
        await getCurrentBlockTimestamp(),
      )

      await jioToken.enableTransfer()
      expect(await jioToken.paused()).to.be.false
    })
    ...
    it("Reverts when attempting to call enableTransfer after it is already unpaused", async () => {
      ...
      await expect(
        jioToken.connect(deployer).enableTransfer(),
      ).to.be.revertedWith("JIO: transfer is enabled")
    })
    ...
    describe("transfer after enableTransfer", () => {
      ...
      it("Succeeds when transferring from a not-allowed address to an allowed address", async () => {
        await jioToken
          .connect(signers[13])
          .transfer(deployerAddress, BIG_NUMBER_1E18.mul(1e8))
        await jioToken.transfer(
          await signers[13].getAddress(),
          BIG_NUMBER_1E18.mul(1e8),
        )
        expect(await jioToken.balanceOf(deployerAddress)).to.eq(BIG_NUMBER_ZERO)
      })
      ...
      it("Succeeds when transferring between not-allowed addresses", async () => {
        ...
        expect(
          await jioToken.balanceOf(await signers[16].getAddress()),
  ...
  describe("addToAllowedList", () => {
    it("Succeeds to add an address to the allowed list", async () => {
      ...
      await jioToken.connect(governance).addToAllowedList([deployerAddress])
      ...
      expect(await jioToken.balanceOf(await signers[16].getAddress())).to.be.eq(
        BIG_NUMBER_1E18.mul(1e8),
      )
    })
    ...
    it("Reverts when called by non-governance", async () => {
      await expect(
        jioToken.connect(deployer).addToAllowedList([deployerAddress]),
  ...
  describe("changeGovernance & acceptGovernance", () => {
    it("Reverts when called by other than the governance", async () => {
      await expect(
        jioToken.connect(deployer).changeGovernance(deployerAddress),
      ).to.be.revertedWith("only governance can perform this action")
    })

    it("Succeeds to change governance", async () => {
      await jioToken.connect(governance).changeGovernance(deployerAddress)
      expect(await jioToken.governance()).to.be.eq(governanceAddress)
      await jioToken.connect(deployer).acceptGovernance()
      expect(await jioToken.governance()).to.be.eq(deployerAddress)
      expect(await jioToken.pendingGovernance()).to.be.eq(ZERO_ADDRESS)
    })

    it("Reverts when accepting governance when changeGovernance is not called before", async () => {
      await expect(
        jioToken.connect(deployer).acceptGovernance(),
    ...
    it("Reverts when accepting governance when called by other than pendingGovernance", async () => {
      await jioToken.connect(governance).changeGovernance(deployerAddress)
      expect(await jioToken.pendingGovernance()).to.be.eq(deployerAddress)
      await expect(
        jioToken.connect(signers[13]).acceptGovernance(),
  ...
  describe("rescueToken", () => {
    ...
    it("Successfully rescues ERC20", async () => {
      ...
      await dummyToken.mint(jioToken.address, BIG_NUMBER_1E18.mul(10000))

      expect(await dummyToken.balanceOf(jioToken.address)).to.eq(
