...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if ((await getOrNull("MiniChefV2")) == null) {
    ...
    // 6 months (24 weeks)
    const lmRewardsPerSecond = TOTAL_LM_REWARDS.div(120 * 4 * 7 * 24 * 3600)

    const batchCall = [
      await minichef.populateTransaction.setJioPerSecond(lmRewardsPerSecond),
